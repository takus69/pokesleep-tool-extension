...
対象は `src/fork/RankingWorkspace.tsx`、`src/fork/RankingScenarioView.tsx`、`src/fork/**` のランキングUI、`src/util/RankingScenario.ts` とその推移的な計算依存、`src/data` の必要データ、翻訳、および既定ランキング設定です。`src/features/ranking/workspace` の派生画面は、元ツール環境とボックスを参照専用にする変更を加えています。現在は隣接する読み取り専用forkを開発時に参照し、Viteが自己完結したcontent scriptへbundleします。ストア公開前に固定commitから再現可能に取得する仕組みへ置き換えます。
